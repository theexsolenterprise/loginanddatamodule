...
interface TierRow {
  ...
  label: string;
  roleAs: RoleAs;
  cap: string; // string for input
  unlimited: boolean;
  bucketing: Bucketing;
  subBucketsMode: SubBuckets; // only meaningful when bucketing === "combined"
  subBucketsN: string; // input value when subBucketsMode === "n"
}

const DEFAULTS: TierRow[] = [
  { key: "store", label: "Store", roleAs: "store", cap: "", unlimited: true, bucketing: "separate", subBucketsMode: "single", subBucketsN: "" },
  { key: "owner", label: "Owner", roleAs: "owner", cap: "", unlimited: true, bucketing: "separate", subBucketsMode: "single", subBucketsN: "" },
  { key: "employee", label: "Employee", roleAs: "employee", cap: "", unlimited: true, bucketing: "separate", subBucketsMode: "single", subBucketsN: "" },
  { key: "customer", label: "Customer", roleAs: "customer", cap: "", unlimited: true, bucketing: "combined", subBucketsMode: "single", subBucketsN: "" },
];
...
/**
 * DynamicTierEditor — fully editable tier list.
 *
 * Admin can: add a tier, remove a tier, rename, change RBAC mapping,
 * set max count or unlimited, choose separate or combined bucketing, and
 * drag to reorder.
 *
 * Output: a single hidden form input `tiers_json` carrying the JSON-encoded
 * array, which the server action parses with Zod into `ClientStructure.tiers`.
 */
export function DynamicTierEditor() {
  const [rows, setRows] = useState<TierRow[]>(DEFAULTS);
  ...
  function add() {
    ...
    let k = base;
    let n = rows.length + 1;
    while (rows.some((r) => r.key === k)) {
      k = `${base}-${n++}`;
    }
    setRows((prev) => [
      ...prev,
      {
        key: k, label: `Tier ${prev.length + 1}`, roleAs: "employee",
  ...
  function previewPiece(r: TierRow): string {
    ...
    if (r.subBucketsMode === "n")
      return `${r.key}/[${r.subBucketsN || "N"} sub-folders]`;
    ...
    <div>
      ...
      <div className="mb-3 flex items-center justify-between">
        <p className="text-xs text-zinc-500">
          Drag <span className="font-medium">⋮⋮</span> to reorder · Top is closest to admin · "Combined" tiers are shared across peers of the tier above.
        </p>
        <button
          type="button"
          onClick={add}
          ...
          return (
            ...
            >
              <div className="grid grid-cols-12 items-end gap-3">
                <span
                  className="col-span-1 cursor-grab self-center select-none text-lg text-zinc-400 hover:text-zinc-700 active:cursor-grabbing"
                ...
                <label className="col-span-2 text-xs text-zinc-600">
                  ...
                  <select
                    value={row.roleAs}
                    onChange={(e) => update(i, { roleAs: e.target.value as RoleAs })}
                    className="mt-1 w-full rounded-md border border-zinc-300 px-2 py-1.5 text-sm"
                  >
                ...
                <div className="col-span-2 text-xs text-zinc-600">
                  ...
                  <input
                    ...
                    placeholder="∞"
                    className="mt-1 w-full rounded-md border border-zinc-300 px-2 py-1.5 text-sm disabled:bg-zinc-50 disabled:text-zinc-400"
                  />
              ...
              {/* Sub-bucket controls — only when bucketing = combined */}
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-4">
                    <label className="inline-flex items-center gap-1">
                      <input
                        type="radio"
                        name={`sub_${row.key}`}
                        checked={row.subBucketsMode === "single"}
                        onChange={() => update(i, { subBucketsMode: "single" })}
                      />
                    ...
                    <label className="inline-flex items-center gap-1">
                      <input
                        type="radio"
                        name={`sub_${row.key}`}
                        checked={row.subBucketsMode === "n"}
                        onChange={() => update(i, { subBucketsMode: "n" })}
                      />
                      ...
                      <input
                        ...
                        placeholder="N"
                        className="ml-1 w-16 rounded-md border border-zinc-300 px-2 py-1 text-xs disabled:bg-zinc-100 disabled:text-zinc-400"
                      />
                    </label>
                    <label className="inline-flex items-center gap-1">
                      <input
                        type="radio"
                        name={`sub_${row.key}`}
                        checked={row.subBucketsMode === "unlimited"}
                        onChange={() => update(i, { subBucketsMode: "unlimited" })}
                      />
      ...
      {rows.length > 0 && (
        <p className="mt-3 rounded-md border border-zinc-200 bg-zinc-50 px-3 py-2 text-xs text-zinc-600">
          Path preview:{" "}
          <code className="font-mono text-zinc-800">{previewPath}</code>
        </p>
      )}
    </div>
  );
